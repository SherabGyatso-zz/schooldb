...
<table  class="table table-bordered table-striped table-hover table-condensed table-responsive">
    <thead >
        ...
        ?>
               <h2 style="color:#f00" align="middle">School Wise Class Enrolment As On <?php echo gmdate("F j, Y", strtotime($_POST['dl'])); ?> </h2>
                </td>
            </tr>
        ...
        <?php
           ...
           $h_student_data = $sch->get_student_total_by_classdetail($dl_cond);
...
$x=1;
           ...
           if (is_array($h_student_data) && count($h_student_data)) {
               ...
               foreach ($h_student_data as $row) {
                if($row['SchoolName'] != $previous_schoolname) {
   $i=1;
  echo '<tr style="color:#00f"><td colspan="20">'.  $x .'. ' . $row['SchoolName'] .'</td></tr>';
  echo '<tbody>
             <tr>
...
<?php
    ...
    $previous_schoolname = $row['SchoolName'];
